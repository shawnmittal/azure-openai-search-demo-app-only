...
parser.add_argument("--storageaccount", help="Azure Blob Storage account URL")
...
parser.add_argument("--searchservice", help="URL of the Azure Cognitive Search service where content should be indexed (must exist already)")
...
parser.add_argument("--formrecognizerservice", required=False, help="Optional. URL of the Azure Form Recognizer service which will be used to extract text, tables and layout from the documents (must exist already)")
...
def upload_blobs(filename):
    blob_service = BlobServiceClient(account_url=args.storageaccount, credential=storage_creds)
...
def get_document_text(filename):
    ...
    if args.localpdfparser:
        ...
        form_recognizer_client = DocumentAnalysisClient(endpoint=args.formrecognizerservice, credential=formrecognizer_creds, headers={"x-ms-useragent": "azure-search-chat-demo/1.0.0"})
...
def create_search_index():
    ...
    index_client = SearchIndexClient(endpoint=args.searchservice,
...
def index_sections(filename, sections):
    ...
    search_client = SearchClient(endpoint=args.searchservice,
